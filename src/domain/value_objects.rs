...
impl Username {
    pub fn new(username: String) -> Result<Self> {
        ...
        if username.len() > 30 {
            return Err(AppError::ValidationError("Username cannot exceed 30 characters".to_string()));
        }
        
        // Username can only contain alphanumeric characters and underscores
        let username_regex = Regex::new(r"^[a-zA-Z0-9_]+$").unwrap();
        if !username_regex.is_match(&username) {
            return Err(AppError::ValidationError("Username can only contain letters, numbers, and underscores".to_string()));
        }
        
        // Username cannot start with underscore
        if username.starts_with('_') {
            return Err(AppError::ValidationError("Username cannot start with underscore".to_string()));
        }
...
impl Email {
    pub fn new(email: String) -> Result<Self> {
        ...
        if email.len() > 254 {
            return Err(AppError::ValidationError("Email cannot exceed 254 characters".to_string()));
        }
...
impl PhoneNumber {
    pub fn new(phone: String) -> Result<Self> {
        ...
        if !phone_regex.is_match(&phone) {
            return Err(AppError::ValidationError("Invalid Nigerian phone number format".to_string()));
        }
...
impl DisplayName {
    pub fn new(name: String) -> Result<Self> {
        ...
        if name.len() > 100 {
            return Err(AppError::ValidationError("Display name cannot exceed 100 characters".to_string()));
        }
...
impl Bio {
    pub fn new(bio: String) -> Result<Self> {
        ...
        if bio.len() > 500 {
            return Err(AppError::ValidationError("Bio cannot exceed 500 characters".to_string()));
        }
