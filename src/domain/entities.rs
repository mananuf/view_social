...
impl MediaAttachment {
    pub fn new(
        ...
        if !url.starts_with("http://") && !url.starts_with("https://") {
            return Err(AppError::ValidationError(
                "Media URL must be a valid HTTP/HTTPS URL".to_string(),
            ));
        }
        ...
        if size <= 0 {
            return Err(AppError::ValidationError(
                "Media size must be positive".to_string(),
            ));
        }

        if size > 100 * 1024 * 1024 {
            return Err(AppError::ValidationError(
                "Media file cannot exceed 100MB".to_string(),
            ));
        }
        ...
        if media_type.starts_with("image/") || media_type.starts_with("video/") {
            if let (Some(w), Some(h)) = (width, height) {
                if w <= 0 || h <= 0 {
                    return Err(AppError::ValidationError(
                        "Media dimensions must be positive".to_string(),
                    ));
                }
                if w > 4096 || h > 4096 {
                    return Err(AppError::ValidationError(
                        "Media dimensions cannot exceed 4096x4096".to_string(),
                    ));
                }
            }
        }
        ...
        if media_type.starts_with("video/") || media_type.starts_with("audio/") {
            if let Some(dur) = duration {
                ...
                if dur > 3600 {
                    return Err(AppError::ValidationError(
                        "Media duration cannot exceed 1 hour".to_string(),
                    ));
                }
            }
        }
...
impl Post {
    pub fn new(request: CreatePostRequest) -> Result<Self> {
        ...
        if request.text_content.is_none() && request.media_attachments.is_empty() {
            return Err(AppError::ValidationError(
                "Post must have either text content or media attachments".to_string(),
            ));
        }
        ...
        if let Some(ref text) = request.text_content {
            ...
            if text.len() > 2000 {
                return Err(AppError::ValidationError(
                    "Text content cannot exceed 2000 characters".to_string(),
                ));
            }
        }
        ...
        if request.is_reel {
            ...
            let has_video = request
                .media_attachments
                .iter()
                .any(|media| media.media_type.starts_with("video/"));

            if !has_video {
                return Err(AppError::ValidationError(
                    "Reels must contain video content".to_string(),
                ));
            }
            ...
            for media in &request.media_attachments {
                if media.media_type.starts_with("video/") {
                    if let Some(duration) = media.duration {
                        if duration > 60 {
                            return Err(AppError::ValidationError(
                                "Reel videos must be under 60 seconds".to_string(),
                            ));
                        }
                    }
                }
        ...
        if request.media_attachments.len() > 10 {
            return Err(AppError::ValidationError(
                "Posts cannot have more than 10 media attachments".to_string(),
            ));
        }
        ...
        let content_type = if request.media_attachments.is_empty() {
            ...
            let has_video = request
                .media_attachments
                .iter()
                .any(|m| m.media_type.starts_with("video/"));
    ...
    pub fn is_video_content(&self) -> bool {
        matches!(
            self.content_type,
            PostContentType::Video | PostContentType::Mixed
        )
    }
    ...
    pub fn is_image_content(&self) -> bool {
        matches!(
            self.content_type,
            PostContentType::Image | PostContentType::Mixed
        )
    }
...
impl PaymentData {
    pub fn new(
        ...
        if currency != "NGN" {
            return Err(AppError::ValidationError(
                "Only NGN currency is supported".to_string(),
            ));
        }
    ...
    pub fn new(request: CreateMessageRequest) -> Result<Self> {
        ...
        match request.message_type {
            MessageType::Text => {
                ...
                {
                    return Err(AppError::ValidationError(
                        "Text messages must have content".to_string(),
                    ));
                }
                ...
                if let Some(ref content) = request.content {
                    if content.len() > 4000 {
                        return Err(AppError::ValidationError(
                            "Message content cannot exceed 4000 characters".to_string(),
                        ));
                    }
                }
            }
            MessageType::Image | MessageType::Video | MessageType::Audio => {
                if request.media_url.is_none()
                    || request.media_url.as_ref().unwrap().trim().is_empty()
                {
                    return Err(AppError::ValidationError(
                        "Media messages must have a media URL".to_string(),
                    ));
                }
            ...
            MessageType::Payment => {
                if request.payment_data.is_none() {
                    return Err(AppError::ValidationError(
                        "Payment messages must have payment data".to_string(),
                    ));
                }

                if request.sender_id.is_none() {
                    return Err(AppError::ValidationError(
                        "Payment messages must have a sender".to_string(),
                    ));
                }
            }
            MessageType::System => {
                if request.sender_id.is_some() {
                    return Err(AppError::ValidationError(
                        "System messages cannot have a sender".to_string(),
                    ));
                }
                ...
                {
                    return Err(AppError::ValidationError(
                        "System messages must have content".to_string(),
                    ));
                }
            }
        }
...
impl Wallet {
    ...
    pub fn verify_pin(&self, pin: &str) -> Result<bool> {
        match &self.pin_hash {
            Some(hash) => bcrypt::verify(pin, hash).map_err(|e| {
                AppError::AuthenticationError(format!("PIN verification failed: {}", e))
            }),
            None => Err(AppError::ValidationError(
                "No PIN set for this wallet".to_string(),
            )),
        }
    }
...
impl Transaction {
    pub fn new(request: CreateTransactionRequest) -> Result<Self> {
        ...
        match request.transaction_type {
            TransactionType::Transfer => {
                if request.sender_wallet_id.is_none() || request.receiver_wallet_id.is_none() {
                    return Err(AppError::ValidationError(
                        "Transfer transactions require both sender and receiver wallets"
                            .to_string(),
                    ));
                }
            ...
            TransactionType::Deposit => {
                if request.receiver_wallet_id.is_none() {
                    return Err(AppError::ValidationError(
                        "Deposit transactions require a receiver wallet".to_string(),
                    ));
                }

                if request.sender_wallet_id.is_some() {
                    return Err(AppError::ValidationError(
                        "Deposit transactions should not have a sender wallet".to_string(),
                    ));
                }
            }
            TransactionType::Withdrawal => {
                if request.sender_wallet_id.is_none() {
                    return Err(AppError::ValidationError(
                        "Withdrawal transactions require a sender wallet".to_string(),
                    ));
                }

                if request.receiver_wallet_id.is_some() {
                    return Err(AppError::ValidationError(
                        "Withdrawal transactions should not have a receiver wallet".to_string(),
                    ));
                }
            }
        }
        ...
        let reference = format!(
            "TXN-{}",
            Uuid::new_v4().to_string().replace('-', "").to_uppercase()[..12].to_string()
        );
