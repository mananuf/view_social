use crate::api::dto::{
    CreatePostRequest, PostDTO, UserDTO, MediaAttachmentDTO, PaginatedResponse,
    SuccessResponse,
};
use crate::api::middleware::AuthUser;
use crate::api::user_handlers::user_to_dto;
use crate::domain::entities::{
    Post, CreatePostRequest as DomainCreatePostRequest, MediaAttachment,
    PostVisibility, PostContentType,
};
...
pub async fn get_feed(
    ...
    let posts = state.post_repo.find_feed(auth_user.user_id, limit, offset).await?;
    
    // Convert posts to DTOs
    let mut post_dtos = Vec::new();
    for post in posts {
        // Get post author
        let author = state.user_repo.find_by_id(post.user_id).await?
            .ok_or_else(|| AppError::NotFound("Post author not found".to_string()))?;
        
        // Check if current user has liked the post
        let is_liked = state.post_repo.has_user_liked(auth_user.user_id, post.id).await?;
...
pub async fn create_post(
    ...
    let visibility = match payload.visibility.as_str() {
        ...
        _ => return Err(AppError::ValidationError("Invalid visibility value".to_string())),
    };
    ...
    // Get post author
    let author = state.user_repo.find_by_id(auth_user.user_id).await?
...
pub async fn like_post(
    ...
    let _post = state.post_repo.find_by_id(post_id).await?
        .ok_or_else(|| AppError::NotFound("Post not found".to_string()))?;
    
    // Check if already liked
    if state.post_repo.has_user_liked(auth_user.user_id, post_id).await? {
        return Err(AppError::Conflict("Post already liked".to_string()));
    }
    
    // Like the post
    state.post_repo.like_post(auth_user.user_id, post_id).await?;
...
pub async fn unlike_post(
    ...
    // Unlike the post
    state.post_repo.unlike_post(auth_user.user_id, post_id).await?;
...
fn post_to_dto(post: &Post, author: &crate::domain::entities::User, is_liked: bool) -> PostDTO {
    ...
    let media_attachments = post.media_attachments.iter().map(|media| {
        MediaAttachmentDTO {
            url: media.url.clone(),
