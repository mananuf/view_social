use crate::api::dto::{
    RegisterRequest, LoginRequest, RefreshTokenRequest, AuthResponse, RefreshTokenResponse,
    UserDTO, SuccessResponse, ErrorResponse,
};
use crate::domain::auth::JwtService;
use crate::domain::entities::{User, CreateUserRequest, CreateWalletRequest};
...
pub async fn register(
    ...
    state.password_service.validate_password(&payload.password)?;
    ...
    if let Some(ref phone) = payload.phone_number {
        if let Some(_) = state.user_repo.find_by_phone_number(phone).await? {
            return Err(AppError::Conflict("Phone number already exists".to_string()));
        }
    }
...
pub async fn login(
    ...
    let user = if payload.username_or_email.contains('@') {
        state.user_repo.find_by_email(&payload.username_or_email).await?
    } else {
        state.user_repo.find_by_username(&payload.username_or_email).await?
    };
    
    let user = user.ok_or_else(|| AppError::AuthenticationError("Invalid credentials".to_string()))?;
    ...
    let is_valid = state.password_service.verify_password(&payload.password, &format!("$2b$12$placeholder_hash_for_{}", user.id))?;
...
pub async fn refresh_token(
    ...
    let new_access_token = state.jwt_service.refresh_access_token(&payload.refresh_token)?;
