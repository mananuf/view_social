...
impl RedisCache {
        ...
        match result {
            Some(serialized) => {
                let value = serde_json::from_str(&serialized)
                    .map_err(|e| AppError::ValidationError(format!("Failed to deserialize cache value: {}", e)))?;
                Ok(Some(value))
            }
            None => Ok(None),
        }
    ...
    pub fn exists(&self, key: &str) -> Result<bool> {
        ...
        let exists: bool = conn.exists(key)
            .map_err(|e| AppError::DatabaseError(format!("Failed to check cache key existence: {}", e)))?;
        Ok(exists)
    }
    ...
    {
        let mut conn = self.get_connection()?;
        let results: Vec<Option<String>> = conn.get(keys)
            .map_err(|e| AppError::DatabaseError(format!("Failed to get multiple cache values: {}", e)))?;
    ...
    pub fn increment(&self, key: &str, by: i64) -> Result<i64> {
        let mut conn = self.get_connection()?;
        let result: i64 = conn.incr(key, by)
            .map_err(|e| AppError::DatabaseError(format!("Failed to increment cache counter: {}", e)))?;
        Ok(result)
    }
    ...
    {
        let mut conn = self.get_connection()?;
        let results: Vec<String> = conn.lrange(key, start, end)
            .map_err(|e| AppError::DatabaseError(format!("Failed to get cache list range: {}", e)))?;
        
        let mut values = Vec::new();
        for serialized in results {
            let value = serde_json::from_str(&serialized)
                .map_err(|e| AppError::ValidationError(format!("Failed to deserialize list value: {}", e)))?;
            values.push(value);
        }
        ...
        T: Serialize,
    {
        ...
        let removed: i64 = conn.lrem(key, 0, serialized)
            .map_err(|e| AppError::DatabaseError(format!("Failed to remove from cache list: {}", e)))?;
...
impl CacheAsidePattern {
    ...
    pub async fn get_or_fetch<T, F, Fut>(&self, key: &str, ttl_seconds: u64, fetch_fn: F) -> Result<T>
    where
    ...
    pub fn invalidate_pattern(&self, pattern: &str) -> Result<()> {
        ...
        let keys: Vec<String> = conn.keys(pattern)
            .map_err(|e| AppError::DatabaseError(format!("Failed to get keys by pattern: {}", e)))?;
...
impl CacheInvalidation {
    ...
    pub fn invalidate_user_caches(&self, user_id: Uuid) -> Result<()> {
        self.cache_aside.invalidate(&CacheKeys::user_profile(user_id))?;
        self.cache_aside.invalidate(&CacheKeys::user_session(user_id))?;
        self.cache_aside.invalidate(&CacheKeys::wallet(user_id))?;
        
        // Invalidate feed caches (user's own feed and feeds of followers)
        self.cache_aside.invalidate_pattern(&format!("feed:user:{}:*", user_id))?;
    ...
    pub fn invalidate_post_caches(&self, post_id: Uuid, author_id: Uuid) -> Result<()> {
        ...
        self.cache_aside.invalidate(&CacheKeys::user_profile(author_id))?;
    ...
    pub fn invalidate_conversation_caches(&self, conversation_id: Uuid) -> Result<()> {
        self.cache_aside.invalidate(&CacheKeys::conversation(conversation_id))?;
        self.cache_aside.invalidate_pattern(&format!("messages:conversation:{}:*", conversation_id))?;
    ...
    pub fn invalidate_follow_caches(&self, follower_id: Uuid, following_id: Uuid) -> Result<()> {
        self.cache_aside.invalidate(&CacheKeys::user_following(follower_id))?;
        self.cache_aside.invalidate(&CacheKeys::user_followers(following_id))?;
        
        // Invalidate user profiles for follower count updates
        self.cache_aside.invalidate(&CacheKeys::user_profile(follower_id))?;
        self.cache_aside.invalidate(&CacheKeys::user_profile(following_id))?;
        
        // Invalidate follower's feed cache
        self.cache_aside.invalidate_pattern(&format!("feed:user:{}:*", follower_id))?;
        
        Ok(())
    }

    /// Invalidate transaction-related caches when payments occur
    pub fn invalidate_transaction_caches(&self, sender_id: Uuid, receiver_id: Uuid, transaction_id: Uuid) -> Result<()> {
        self.cache_aside.invalidate(&CacheKeys::wallet(sender_id))?;
        self.cache_aside.invalidate(&CacheKeys::wallet(receiver_id))?;
        self.cache_aside.invalidate(&CacheKeys::transaction(transaction_id))?;
